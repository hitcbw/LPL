...
from .graph.DeST.dest_graph import Graph
from .graph.DeST.dest_tools import k_adjacency, normalize_adjacency_matrix, get_adjacency_matrix
...
class MultiScale_GraphConv(nn.Module):
    def __init__(self,
        ...
        self.graph = Graph(labeling_mode='spatial', layout=dataset) #拿到图
        neighbor = self.graph.neighbor
        self.num_scales = num_scales

        A_binary = get_adjacency_matrix(neighbor, kwargs['n_node'])
        ...
        if use_mask:
            ...
            self.A_res = nn.init.uniform_(nn.Parameter(torch.Tensor(self.A_powers.shape)), -1e-6, 1e-6) #一个很小的mask

        self.mlp = MLP(in_channels * num_scales, [out_channels], dropout=dropout, activation=activation) #一个通道变换的mlp
